...
class CopyDispatcher(BaseDispatcher):
    ...
    def execute(
        ...
        for source in sources_files:
            ...
            try:
                # send message
                dcm_id = source.get("dcm_id", "undefined")
...
def transfer_file_message(
    ...
    my_dcm_id = dcm_id if dcm_id else 'undefined'
    payload = {
...
class EOperationType(enum.Enum):
    A = 0  # copy to greenroom RAW, straight copy ConfigClass.GREENROOM_ROOT_PATH + '/' + self.project + '/processed/' + file_name
    B = 1  # copy to core RAW, publish data to ConfigClass.CORE_ROOT_PATH + '/' + self.project + '/raw/' + filename


#def interpret_operation_location(file_name, project, destination) -> str:
    #return os.path.join(ConfigClass.CORE_ROOT_PATH, project, destination, file_name)
